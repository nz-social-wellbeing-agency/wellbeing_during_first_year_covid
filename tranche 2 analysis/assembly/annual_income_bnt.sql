...
Output:
- [IDI_Sandpit].[DL-MAA2021-55].[defn_all_income]
...
5. In the table [IDI_Clean_202203].[msd_clean].[msd_third_tier_expenditure], we only consider [msd_tte_recoverable_ind] = 'N', 
...
2. runtime for [IDI_Sandpit].[DL-MAA2021-55].[tmp_benefit_tier2]:  18 mins
 

History (reverse order):
2022-06-30 SA adjust to wellbeing research
2021-06-10 SA QA
2021-02-03 FL v3 
2021-01-26 SA QA
...
DROP VIEW IF EXISTS [DL-MAA2021-55].[temp_income_tax_year];
GO

CREATE VIEW [DL-MAA2021-55].[temp_income_tax_year] AS
SELECT *
...
WHERE [inc_tax_yr_sum_year_nbr] >= 2015
GO
...
DECLARE @y_min INT = 2015
DECLARE @y_max INT = 2022
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2021-55].[tmp_benefit_tier2];
GO
WITH dates_breakdown AS (
	...
	FROM  [IDI_Clean_202203].[msd_clean].[msd_second_tier_expenditure] t1
	INNER JOIN #tax_year  t2
	ON msd_ste_start_date < tax_year_end AND tax_year_start < msd_ste_end_date
	WHERE YEAR([msd_ste_start_date]) >= 2015
),
...
INTO [IDI_Sandpit].[DL-MAA2021-55].[tmp_benefit_tier2]
FROM payment_breakdown
GROUP BY [snz_uid], [yr_tax]

CREATE INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2021-55].[tmp_benefit_tier2] ([snz_uid]);
GO

 /*********************************************************************
 third tier benefits by tax year
 *********************************************************************/
DROP VIEW IF EXISTS [DL-MAA2021-55].[tmp_benefit_tier3];
GO

CREATE VIEW [DL-MAA2021-55].[tmp_benefit_tier3] AS
SELECT [snz_uid]
	  ,DATEFROMPARTS([yr_tax],  1,  1) AS [start_date]
	  ,DATEFROMPARTS([yr_tax], 12, 31) AS [end_date]
...
FROM(
	...
	AND YEAR([msd_tte_decision_date]) >= 2015 
) tier3
GROUP BY [snz_uid], [yr_tax]
GO
 ...
 *********************************************************************/
DROP VIEW IF EXISTS [DL-MAA2021-55].[tmp_wff];
GO

CREATE VIEW [DL-MAA2021-55].[tmp_wff] AS
SELECT [snz_uid]  
      ,[tax_year]
...
FROM(
	...
	FROM [DL-MAA2021-55].[tmp_wff]
 ) a
GO

CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2021-55].[tmp_wff_neg_adj] (snz_uid);
GO
...
DROP TABLE IF EXISTS [IDI_Sandpit].[DL-MAA2021-55].[defn_all_income];
GO
...
INTO [IDI_Sandpit].[DL-MAA2021-55].[defn_all_income]
FROM [DL-MAA2021-55].[temp_income_tax_year] ir
FULL JOIN [IDI_Sandpit].[DL-MAA2021-55].[tmp_benefit_tier2] tier2
ON ir.[snz_uid] = tier2.[snz_uid]
AND ir.[start_date] = tier2.[start_date]
AND ir.[end_date] = tier2.[end_date]
FULL JOIN [DL-MAA2021-55].[tmp_benefit_tier3] tier3
ON ir.[snz_uid] = tier3.[snz_uid]
AND ir.[start_date] = tier3.[start_date]
AND ir.[end_date] = tier3.[end_date] 
FULL JOIN [IDI_Sandpit].[DL-MAA2021-55].[tmp_wff_neg_adj] wff
...
CREATE NONCLUSTERED INDEX my_index_name ON [IDI_Sandpit].[DL-MAA2021-55].[defn_all_income] (snz_uid, start_date, end_date);
GO

/* Compress to save space  (takes 12 minutes) */
ALTER TABLE [IDI_Sandpit].[DL-MAA2021-55].[defn_all_income] REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
